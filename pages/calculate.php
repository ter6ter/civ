...
if ($user->turn_status == 'play') {
    ...
    if ($game->turn_type == 'byturn' || $game->turn_type == 'onewindow') {
        $next_user = MyDB::query("SELECT id FROM user WHERE game = ?gid AND turn_order > ?turnorder LIMIT 1",
            ['gid' => $game->id, 'turnorder' => $user->turn_order], 'elem');
        //Следующий игрок, который должен ходить
        if ($next_user) {
